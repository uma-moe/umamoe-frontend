...
describe('CaratPlannerCalculationService', () => {
  ...
  it('counts a bundled login bonus at its first claim date', () => {
    const plan = makePlan({
      projectionStartDate: '2026-08-26',
      enabledRewardIds: ['global-news-994-login-bonus'],
    });
    const data: CaratPlannerDataBundle = {
      ...emptyData(),
      rewards: { rewards: [{
        ...
        available_until: '2026-12-30T14:59:00Z',
      }] },
    };
    ...
    expect(ledger.map(entry => [entry.id, entry.date, entry.amount])).toEqual([
      ['reward:global-news-994-login-bonus:free_jewels', '2026-08-26', 3300],
    ]);
  });
