...
export class DatabaseFilterComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private readonly friendlyFieldAliases: FriendlyFieldAlias[] = [
    ...
    { label: 'Scenario', aliases: ['scenario', 'training scenario', 'scenario id', 'scenario_id'], field: 'scenario_id', type: 'number' },
  ...
  loadSerializedState(
    ...
    options: { emitImmediately?: boolean; persist?: boolean } = {},
  ) {
    try {
      ...
      if (this.filterMode === 'uql') {
        ...
        this.onUqlChange({ emitImmediately: options.emitImmediately, persist: shouldPersist });
        return;
      }
  ...
  onUqlChange(options: { emitImmediately?: boolean; persist?: boolean } = {}): void {
    this.applyUqlEditorDirectives();
    this.validateUqlQuery();
    this.syncUqlFilterState();
    this.syncSelectedVeteranFilterState();
  ...
  private resolveScenarioUqlValue(rawValue: string): string | null {
    ...
    return this.scenarioOptionIds
      .find(id => this.normalizeUqlName(this.getScenarioName(id)) === normalizedValue)
      ?.toString() ?? null;
  }
